...
//============================================================================================
struct WARN_BITS {
	Uint16 TA0 :1;		//Udc¹ÊÕÏ
	Uint16 TA1 :1;		//Idc¹ýÁ÷
	Uint16 TA2 :1;		// Idc²ÉÑù
	Uint16 TA3 :1;		// Udc²ÉÑù
	Uint16 TA4 :1;		// DSP¹ýÔØ
	Uint16 TA5 :1;		//IA¹ýÁ÷
	Uint16 TA6 :1;		//IB¹ýÁ÷
	Uint16 TA7 :1;		//IC¹ýÁ÷
	Uint16 TA8 :1;		// Iac²»Æ½ºâ
	Uint16 TA9 :1;		// IaÈ±Ïà
	Uint16 TA10 :1;		// IbÈ±Ïà
	Uint16 TA11 :1;		// IcÈ±Ïà
	Uint16 TA12 :1;		// Ia²ÉÑù
	Uint16 TA13 :1;		// Ib²ÉÑù
	Uint16 TA14 :1;		// Ic²ÉÑù
	Uint16 TA15 :1;		// Í¬²½
};
union WARN_REG {
	Uint16 all;
...
//==============================================================================
interrupt void DPRAM_isr_Fix(void) //ï¿½Ì¶ï¿½ï¿½Ð¶ï¿½Æµï¿½ï¿½4.35kHz
{
	/**/
	PX_Out_Spf.NX_DspPlCn++;
	if (PX_Out_Spf.NX_DspPlCn > 32767)
...
void DPRAM_RD(void) //MCU-->DSP
{
	...
	DspData.XU_PhABLk = *(XintfZone7 + 0x7) * 0.1 * 2.0;		// AB
	Ext_U = *(XintfZone7 + 0x11) * 10.0;		//ï¿½ï¿½Ñ¹
	Ext_F = *(XintfZone7 + 0x12) * 1.0;		//Æµï¿½ï¿½
	TestFlg = *(XintfZone7 + 0x1A);		//0x55Îªï¿½ï¿½ï¿½ï¿½
}
//==============================================================================
...
//	*(XintfZone7 + 0x2A) = fabs(DspData.XU_3PhAbs/SQRT2*10 );
	...
	*(XintfZone7 + 0x2F) = DA[7];
	...
	//---------------------------------------------------
	*(XintfZone7 + 0x7FFE) = PX_Out_Spf.NX_DspPlCn;	// ï¿½Ò²ï¿½Ð´7FFEï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ð¶ï¿½
	//------------------------------------------------------------
...
void DspStCl1(void) {
	...
	if (PX_Out_Spf.oldDspSt.bit.CvSt == 0x00) {
		...
		if (McuData.C_CvOpSa_MnSq && (PX_In_Spf.XX_McuFlag1.bit.CvOp == 1)) {
			PX_Out_Spf.NX_DspOpSt.bit.CvSt = 0x40;
		}
	...
	/**/
	if (PX_Out_Spf.oldDspSt.bit.OvpCp == 0x0) {
		if (McuData.C_DcLkCh && (PX_In_Spf.XX_McuFlag1.bit.OvpFcTsAv == 1))
			PX_Out_Spf.NX_DspOpSt.bit.OvpCp = 0x1;
	} else if (PX_Out_Spf.oldDspSt.bit.OvpCp == 0x1) {
		PX_Out_Spf.XX_DspFlag1.bit.OvpFcTs = 0;	//
		PX_Out_Spf.XX_DspFlag1.bit.OvpCpAv = 1;
	...
	//-------------
	//BtCp
	PX_Out_Spf.NX_DspOpSt.bit.BtCp = 0x0;

	PX_Out_Spf.oldDspSt.all = PX_Out_Spf.NX_DspOpSt.all;
	PX_Out_Spf.XX_DspFlag1.bit.CdAuLdCt = McuData.C_CdAuLdCt_ALCS;
}
//===============================================================================================
//=========================================THE END===============================================
//===============================================================================================
