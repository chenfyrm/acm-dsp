...
//==============================================================================
interrupt void DPRAM_isr_Fix(void) //�̶��ж�Ƶ��4.35kHz
{
	/*�����ֳ�*/
...
void DPRAM_RD(void) //MCU-->DSP
{
	...
	DspData.XU_PhABLk = *(XintfZone7 + 0x7) * 0.1 * 2.0;		// AB
	Ext_U = *(XintfZone7 + 0x11) * 10.0;		//��ѹ
	Ext_F = *(XintfZone7 + 0x12) * 1.0;		//Ƶ��
	TestFlg = *(XintfZone7 + 0x1A);		//0x55Ϊ����
}
//==============================================================================
...
//	*(XintfZone7 + 0x2A) = fabs(DspData.XU_3PhAbs/SQRT2*10 );
	...
	*(XintfZone7 + 0x2F) = DA[7];
	...
	//---------------------------------------------------
	*(XintfZone7 + 0x7FFE) = PX_Out_Spf.NX_DspPlCn;		// �Ҳ�д7FFE���������ж�
	//------------------------------------------------------------
