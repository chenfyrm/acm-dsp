...
//		15	预留
	...
	//初始化
		...
		{
			if((PX_In_Spf.NX_McuOpSt == 0x408)&&(PX_In_Spf.XX_McuFlag1.bit.CvOp == 1))
	...
	//------------------------------------------------------------
		...
		{
			if(PX_In_Spf.XX_McuFlag1.bit.CvOp == 0)
		...
		{
			if((PX_In_Spf.NX_McuOpSt == 0x405)&&(PX_In_Spf.XX_McuFlag1.bit.OvpAv == 1)&&(PX_Out_Spf.oldDspSt.bit.CvSt == 0x30))
				PX_Out_Spf.NX_DspOpSt.bit.OvpCp = 0x1;
		}
		else if(PX_Out_Spf.oldDspSt.bit.OvpCp == 0x1)
		{
			//
			PX_Out_Spf.oldDspSt.bit.OvpCp == 0x2;
		}
