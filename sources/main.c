...
//==============================================================================
interrupt void DPRAM_isr_Fix(void) //锟教讹拷锟叫讹拷频锟斤拷4.35kHz
{
	/**/
	PX_Out_Spf.NX_DspPlCn++;
	if (PX_Out_Spf.NX_DspPlCn > 32767)
...
void DPRAM_RD(void) //MCU-->DSP
{
	...
	DspData.XU_PhABLk = *(XintfZone7 + 0x7) * 0.1 * 2.0;		// AB
	Ext_U = *(XintfZone7 + 0x11) * 10.0;		//锟斤拷压
	Ext_F = *(XintfZone7 + 0x12) * 1.0;		//频锟斤拷
	TestFlg = *(XintfZone7 + 0x1A);		//0x55为锟斤拷锟斤拷
}
//==============================================================================
...
//	*(XintfZone7 + 0x2A) = fabs(DspData.XU_3PhAbs/SQRT2*10 );
	...
	*(XintfZone7 + 0x2F) = DA[7];
	...
	//---------------------------------------------------
	*(XintfZone7 + 0x7FFE) = PX_Out_Spf.NX_DspPlCn;	// 锟揭诧拷写7FFE锟斤拷锟斤拷锟斤拷锟斤拷锟叫讹拷
	//------------------------------------------------------------
